<template>
  <div class="debug-panel">
    ...
    <div v-if="expanded" class="debug-body">
      ...
      <!-- Секция: Пресеты -->
      <div class="debug-section">
        <div class="debug-section-title" @click="showPresets = !showPresets">
          ⚡ Пресеты {{ showPresets ? '▼' : '▶' }}
        </div>
        <div v-if="showPresets" class="debug-section-body">
          <div class="debug-presets">
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetExitBase">🚪 Выход из базы</button>
              <button class="debug-btn-sm debug-btn-hint" @click="hintExitBase = !hintExitBase">
                ❓
              </button>
              <div v-if="hintExitBase" class="debug-preset-hint">
                Фишка на базе, кости [5,3]. Кликнуть фишку → выходит на main[4].
              </div>
            </div>
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetCapture">⚔️ Захват</button>
              <button class="debug-btn-sm debug-btn-hint" @click="hintCapture = !hintCapture">
                ❓
              </button>
              <div v-if="hintCapture" class="debug-preset-hint">
                Своя main[10], чужая main[15], кости [5,2]. Шаг 5 → захват → бонус +20.
              </div>
            </div>
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetBlockade">🧱 Блокада</button>
              <button class="debug-btn-sm debug-btn-hint" @click="hintBlockade = !hintBlockade">
                ❓
              </button>
              <div v-if="hintBlockade" class="debug-preset-hint">
                Две своих на main[10], чужая на main[5], ход противника. Не может пройти сквозь
                main[10].
              </div>
            </div>
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetFinish">🏁 Финиш</button>
              <button class="debug-btn-sm debug-btn-hint" @click="hintFinish = !hintFinish">
                ❓
              </button>
              <div v-if="hintFinish" class="debug-preset-hint">
                Фишка на home[6], кости [2,1]. Шаг 2 → финиш → бонус +10.
              </div>
            </div>
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetThreeDoubles">🎲 3 дубля</button>
              <button
                class="debug-btn-sm debug-btn-hint"
                @click="hintThreeDoubles = !hintThreeDoubles"
              >
                ❓
              </button>
              <div v-if="hintThreeDoubles" class="debug-preset-hint">
                Счётчик дублей = 2, кости [3,3]. Бросок → 3-й дубль → фишка на базу.
              </div>
            </div>
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetBonus20">💎 Бонус 20</button>
              <button class="debug-btn-sm debug-btn-hint" @click="hintBonus20 = !hintBonus20">
                ❓
              </button>
              <div v-if="hintBonus20" class="debug-preset-hint">
                Кости [3,4] использованы, бонус [20]. В списке шагов появляется +20.
              </div>
            </div>
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetAllOnMain">🔄 Все на дорожке</button>
              <button class="debug-btn-sm debug-btn-hint" @click="hintAllOnMain = !hintAllOnMain">
                ❓
              </button>
              <div v-if="hintAllOnMain" class="debug-preset-hint">
                4 фишки на main[4,10,25,40], кости [4,3]. Все фишки подсвечены как доступные.
              </div>
            </div>
            <div class="debug-preset-item">
              <button class="debug-btn" @click="presetBlockedEntrance">
                🚫 Заблокированный вход
              </button>
              <button
                class="debug-btn-sm debug-btn-hint"
                @click="hintBlockedEntrance = !hintBlockedEntrance"
              >
                ❓
              </button>
              <div v-if="hintBlockedEntrance" class="debug-preset-hint">
                Две чужих на main[67] (вход на финиш), своя на main[60]. Нельзя повернуть на финиш.
              </div>
            </div>
          </div>
        </div>
...
// Подсказки для пресетов
const hintExitBase = ref(false);
const hintCapture = ref(false);
const hintBlockade = ref(false);
const hintFinish = ref(false);
const hintThreeDoubles = ref(false);
const hintBonus20 = ref(false);
const hintAllOnMain = ref(false);
const hintBlockedEntrance = ref(false);
...
const presetExitBase = () => {
  // Фишка игрока 0 на main[4], кости [5, 3]
  const player = playerStore.players[0]!;
  const chip = player.chips[0]!;
  // Отправляем фишку на базу (если не там)
  if (chip.cell?.board.type !== BoardType.base) {
    chip.go(player.baseBoard.cells[0]!);
  }
  // Устанавливаем кости
  debugDice.value = [5, 3];
  debugUsedText.value = '';
  debugBonusText.value = '';
  debugDoublesCount.value = 0;
  debugStateId.value = GameStateEnum.WAIT_STEP;
  applyDiceState();
  // Устанавливаем текущего игрока
  playerStore.init(0);
  gameStore.debugLogPush(
    'preset',
    'Пресет: Выход из базы — фишка #' + chip.id + ' на базе, кости [5,3]',
    'success',
  );
};

const presetCapture = () => {
  // Фишка A (игрок 0) на main[10], фишка B (игрок 1) на main[15], кости [5, 2]
  const p0 = playerStore.players[0]!;
  const p1 = playerStore.players[1]!;
  const chipA = p0.chips[0]!;
  const chipB = p1.chips[0]!;

  // Размещаем фишки
  chipA.go(boardStore.board.cells[10]!);
  chipB.go(boardStore.board.cells[15]!);
  // Остальные фишки на базу
  for (let i = 1; i < 4; i++) {
    p0.chips[i]!.go(p0.baseBoard.cells[0]!);
    p1.chips[i]!.go(p1.baseBoard.cells[0]!);
  }
  ...
  gameStore.debugLogPush(
    ...
    'Пресет: Захват — фишка #' +
      ...
      ' на main[10], фишка #' +
      chipB.id +
      ' на main[15], кости [5,2]',
    'success',
  );
};

const presetBlockade = () => {
  // Две фишки игрока 0 на main[10], фишка игрока 1 пытается пройти
  const p0 = playerStore.players[0]!;
  const p1 = playerStore.players[1]!;
  const chipA = p0.chips[0]!;
  const chipB = p0.chips[1]!;
  const chipC = p1.chips[0]!;

  // Блокада: две фишки одного цвета на main[10]
  chipA.go(boardStore.board.cells[10]!);
  chipB.go(boardStore.board.cells[10]!);
  // Фишка противника на main[5], хочет пройти на 5 шагов
  chipC.go(boardStore.board.cells[5]!);
  // Остальные на базу
  for (let i = 2; i < 4; i++) {
    p0.chips[i]!.go(p0.baseBoard.cells[0]!);
    p1.chips[i]!.go(p1.baseBoard.cells[0]!);
  ...
  gameStore.debugLogPush(
    'preset',
    'Пресет: Блокада — две фишки на main[10], фишка противника на main[5]',
    'success',
  );
};

const presetFinish = () => {
  // Фишка на homeBoard[6], кости [2, 1] — можно финишировать
  const p0 = playerStore.players[0]!;
  const chip = p0.chips[0]!;
  chip.go(p0.homeBoard.cells[6]!);
  // Остальные на базу
  for (let i = 1; i < 4; i++) {
    p0.chips[i]!.go(p0.baseBoard.cells[0]!);
  }
  ...
  applyDiceState();
  playerStore.init(0);
  gameStore.debugLogPush('preset', 'Пресет: Финиш — фишка на home[6], кости [2,1]', 'success');
};

const presetThreeDoubles = () => {
  // Устанавливаем счётчик дублей = 2, кости [3, 3]
  const p0 = playerStore.players[0]!;
  const chip = p0.chips[0]!;
  chip.go(boardStore.board.cells[20]!);

  debugDice.value = [3, 3];
  debugUsedText.value = '';
  debugBonusText.value = '';
  debugDoublesCount.value = 2;
  debugStateId.value = GameStateEnum.WAIT_ROLL;
  applyDiceState();
  playerStore.init(0);
  gameStore.debugLogPush(
    'preset',
    'Пресет: 3 дубля — счётчик=2, сейчас будет 3-й дубль',
    'warning',
  );
};

const presetBonus20 = () => {
  // После захвата: кости использованы, есть бонус +20
  const p0 = playerStore.players[0]!;
  const chip = p0.chips[0]!;
  chip.go(boardStore.board.cells[10]!);

  debugDice.value = [3, 4];
  debugUsedText.value = '3,4';
  debugBonusText.value = '20';
  debugDoublesCount.value = 0;
  debugStateId.value = GameStateEnum.WAIT_STEP;
  applyDiceState();
  playerStore.init(0);
  gameStore.debugLogPush(
    'preset',
    'Пресет: Бонус 20 — кубики использованы, бонус +20 доступен',
    'success',
  );
};

const presetAllOnMain = () => {
  // Все 4 фишки игрока 0 на разных ячейках основной доски
  const p0 = playerStore.players[0]!;
  p0.chips[0]!.go(boardStore.board.cells[4]!);
  p0.chips[1]!.go(boardStore.board.cells[10]!);
  p0.chips[2]!.go(boardStore.board.cells[25]!);
  p0.chips[3]!.go(boardStore.board.cells[40]!);
  ...
  applyDiceState();
  playerStore.init(0);
  gameStore.debugLogPush('preset', 'Пресет: Все на дорожке — 4 фишки на main', 'success');
};

const presetBlockedEntrance = () => {
  // Вход на финиш (main[67] для игрока 0) заблокирован двумя фишками
  const p0 = playerStore.players[0]!;
  const p1 = playerStore.players[1]!;
  // Две фишки противника на входе на финиш игрока 0 (main[67])
  p1.chips[0]!.go(boardStore.board.cells[67]!);
  p1.chips[1]!.go(boardStore.board.cells[67]!);
  // Фишка игрока 0 на main[60], хочет дойти до финиша
  p0.chips[0]!.go(boardStore.board.cells[60]!);
  ...
  gameStore.debugLogPush(
    'preset',
    'Пресет: Заблокированный вход — main[67] занят двумя чужими фишками',
    'warning',
  );
};
