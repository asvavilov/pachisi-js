...
| 6   | `hasStart` — true если есть 5 (выход с базы)`                   | Устаревшее «6» заменено на сумму 5        |
| 7   | `hasAddon` — true при дубле`                                    | Доп. бросок даёт дубль (не «6»)           |
...
| 16  | `handleThreeDoubles()` отправляет последнюю двинутую фишку на базу (README п.7)` |           |
...
| 53  | `moveChip()` выход на занятую стартовую («расплющивание») → без бонуса +20 (README п.11)` |          |
