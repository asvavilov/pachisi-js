...
| 91  | `isCellDisabled()` чужая стартовая клетка, занятая владельцем, для чужака → true` | Реализация: `isSafeCell` возвращает true только для владельца; но `isCellDisabled` проверяет `isSafeCell(cell, occupyingPlayer)` — т.е. для владельца клетка disabled. Чужак не может встать. **FIXME: противоречит README п.9** — на безопасных/стартовых клетках могут стоять фишки разных цветов |
| 92  | `isCellDisabled()` общая безопасная клетка (safe=true), занятая одним цветом → true` | Реализация блокирует постановку второго цвета. **FIXME: противоречит README п.9** |
