...
| 57  | **Крайний случай:** захват на безопасной ячейке — не происходит`                                              | Безопасная клетка защищает от захвата (README п.9, 10)              |
...
| 3   | Три дубля подряд → последняя двинутая фишка на базу (на цветной дорожке — игнор)              |             |
| 4   | Блокада: две фишки одного цвета → никто не может пройти                                       |             |
| 5   | Выпал дубль → дополнительный бросок                                                           |             |
| 6   | Правило «+7»: все 4 фишки в игре + дубль → перемещение на 7 клеток                            |             |
| 7   | Барьер + дубль → игрок обязан сдвинуть одну из фишек барьера                                  |             |
| 8   | Выход с базы на занятую стартовую → чужие фишки на базу без бонуса («расплющивание»)          |             |
| 9   | Все 4 фишки в базе + сумма 5 → выводятся сразу две фишки на стартовую                         | README п.4  |
| 10  | После первого победителя игра продолжается → разыгрываются 2-е и 3-е места                    | README п.14 |
