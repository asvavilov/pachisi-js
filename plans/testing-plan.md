...
> ## 📌 Статус выполнения
>
> **Vitest настроен и работает.** Все юнит-тесты из разделов 3–5 написаны и **проходят (202 теста / 10 файлов)**.
...
> | 3. Тесты `src/lib/`        | ✅ Готово    | 56     |
> | 4. Тесты `src/stores/`     | ✅ Готово    | 142    |
...
> **Результат последнего прогона:** `Test Files 10 passed (10)`, `Tests 202 passed (202)`.
...
| 57  | **Крайний случай:** захват на безопасной ячейке — не происходит`                                              | Безопасная клетка защищает от захвата (README п.9, 10). **FIXME:** в текущей реализации `moveChip` не проверяет `isSafeCell` перед захватом — захват на безопасной клетке возможен. Тест зафиксирует ожидаемое поведение (захват не происходит), что потребует исправления `moveChip` |
